# Release Workflows & Tool Reference

Read this when running the end-to-end release process (changelog, version bump, readiness), applying version-bump or rollback rules, troubleshooting the scripts, or looking up full CLI flags for `changelog_generator.py`, `version_bumper.py`, and `release_planner.py`.
